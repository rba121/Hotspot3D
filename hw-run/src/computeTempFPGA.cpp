...
void computeTempFPGA(float *pIn, float* tIn, float *tOut, 
		float Cap, float Rx, float Ry, float Rz, float dt, int numiter) { float ce, cw, cn, cs, ct, cb, cc;

	#pragma HLS interface m_axi port=pIn offset=slave bundle=gmem
	#pragma HLS interface m_axi port=tIn offset=slave bundle=gmem
	#pragma HLS interface m_axi port=tOut offset=slave bundle=gmem
	...
	#pragma HLS array_partition variable=powerBuffer cyclic factor=8 dim=1

	float resultBuffer[TILE_COLS];
	#pragma HLS array_partition variable=resultBuffer cyclic factor=8 dim=1
